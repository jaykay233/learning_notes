...
这个动画不会让整行地址滑到另一个位置。它发生的是 bit 字段置换，因此
不应该把相邻阶段互相比较。正确方法是只比较：
...
| 字段 | `m` | `addr` | 是否变化 |
|---|---:|---:|---|
| `row`，bits 8..6 | `101` | `101` | 不变 |
| `q`，bits 5..3 | `000` | `101` | `000 XOR row` |
| `low`，bits 2..0 | `000` | `000` | 不变 |
...
如果可视化同时在 `m` 和 `addr` 两行标出变化位置，会看到四个红色
格子。它们表示两个变化位置各自的“前值”和“后值”：

```text
两个 bit 位置 * 前后两格 = 四个高亮格
```
...
1. 先看最后的 `addr`，不要只看中间的 `x`、`source` 或 `x2`。
2. 按 `row / q / low` 分组，判断哪个字段发生变化。
3. 对变化的 `q` 字段逐 bit 计算 XOR。
4. 最后检查 bank 标签，本例从 `0` 变为 `20`。

最容易产生的误解，是把中间阶段也当成最终状态：
...
前四个值包含移位和截取，因此看起来有很多 bit 在变化。真正应该比较的
是输入 `m` 和最终 `addr`：
...
```text
bank(m) = floor(320 / 2) mod 32
        = 0

bank(addr) = floor(360 / 2) mod 32
           = 20
```

也就是说，动画是否“发生了变化”有两个直接判据：

```text
最终 addr 是否不同于输入 m
最终 bank 是否不同于输入 bank
```

本例两者都发生了变化；变化只发生在 `q` 字段。
