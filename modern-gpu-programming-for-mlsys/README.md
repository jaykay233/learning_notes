...
hgemm_v5 为 A/B 增加 PIPE_DEPTH=2 的 SMEM ring buffer
prologue 预取前两个 stages，main loop 用 stage = k % PIPE_DEPTH 复用 slot
每个 stage 使用独立 tma_bar，phase_tma 在 ring wrap 时翻转
MMA 仍共用一个 tmem accumulator 和 mma_bar，phase_mma 每轮翻转
...
Two-CTA cluster 以 256 x 256 output tile 为调度单位
CTA0 加载 A rows 0:128 与 stored-B rows 0:128，CTA1 加载 A/B rows 128:256
cooperative MMA 跨 CTA 读取 SMEM，并产生四个 128 x 128象限
CTA0 的 tma2mma 集中登记两个 CTA 的 65536-byte K-stage transaction
...
CTA0 的 tma2mma barrier 作为两个 CTA 的集中 completion point
tma2mma.init(1) 统计一次软件 arrival，不统计 CTA 或 TMA engine 数量
两个 CTA 的 A/B TMA completion 都通过 remote_view(0) 报告到 CTA0
每个 K stage 的 tx_count 是 2 * (128*64 + 128*64) * 2 = 65536 bytes
