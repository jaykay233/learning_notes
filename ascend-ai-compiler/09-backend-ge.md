...
```text
1. Parser            — 框架 → 中间图（偏前端，见 08）
2. 图准备            — 原图优化、Infershape
3. 图拆分            — 按引擎（AI Core / AI CPU…）切子图
4. 图优化            — 子图内融合、选实现、再整图收一遍  ← 最像「算子融合」
5. 图编译            — 内存 / stream / task → 写 *.om
```

**最像算子融合的是第 4 步「图优化」**（尤其子图优化：UB 融合、匹配算子库把多 op 收成一截高效实现）。  
整图优化也会再做跨边界的收束，但仍归在「图优化」这一步，而不是拆分或最终写 om。

**2. 为什么说 om 是 AOT 产物？**

**AOT = Ahead-Of-Time**：重活（选 kernel、内存规划、调度、打包）在 **上线前 / 设备执行前** 由 ATC+GE 做完，结果固化进 `*.om`。

运行时（AscendCL / msame）主要是：加载 om → 填输入 → Execute，**一般不再做一轮完整图编译**。  
这与「每次请求现编译」的 JIT 相对；课上 Lab2–6 主路径就是这种 AOT。

（对比记忆：Transform 产出 om；Deploy 消费 om。）

**3. TopToTpu 与 GE 图优化——一句话类比**

**TopToTpu ≈ 把「框架无关的 Top 意图」降到「硬件相关的 Tpu 方言」；GE 的图拆分/优化/编译 ≈ 昇腾上干同一件事：中间图 → 绑引擎与 kernel → 编排资源 → 写出 om。**
